<div>
    ...
    <x-table>
        <x-slot name="thead">
            <tr>
                ...
                <x-th  class="text-center" value="Editar" />
            </tr>
        </x-slot>
        <x-slot name="tbody">
            @if ($users)
                @foreach ($users as $user)
                    <x-tr>
                        <x-td value="{{ $user->nickname }}" />
                        <x-td value="{{ $user->name . ', ' . $user->lastname }}" />
                        <x-td value="{{ $user->email }}" />
                        <x-td value="{{ $user->role_id }}" />
                        <x-td value="0" />
                        <x-td class="text-center">
                            <x-danger-button wire:click="confirmDelete({{ $user->id }})" class="ml-1">
                                <i class="icon icon-remove"></i> Quitar del Proyecto
                            </x-danger-button>
                        </x-td>
                    </x-tr>
                @endforeach
    ...
    <x-confirmation-modal id="confirmDeleteModal" wire:model="isDeleting">
        <x-slot name="title">
            Confirmar Quitar del proyecto
        </x-slot>
        <x-slot name="content">
            ¿Estás seguro que deseas quitar este usuario del proyecto?
        </x-slot>
        <x-slot name="footer">
